...
public abstract class Block extends BlockPosition implements Metadatable, Cloneable, AxisAlignedBB {
    ...
    public final boolean canBeFloodedWithWaterlogging() {
        return canBeFlooded() || getWaterloggingLevel() > 1;
    }
    ...
    public Block getSideAtLayer(int layer, BlockFace face) {
        return getSideAtLayer(layer, face, true);
    }
    
    public Block getSideAtLayer(int layer, BlockFace face, boolean load) {
        if (this.isValid()) {
            return this.getLevel().getBlock(x + face.getXOffset(), y + face.getYOffset(), z + face.getZOffset(), layer);
        }
            ...
            if (step == 1) {
                if (load) {
                    ...
                    return this.getLevel().getLoadedBlock(x + face.getXOffset(), y + face.getYOffset(),
                            z + face.getZOffset(), layer);
                }
            } else {
                if (load) {
                    return this.getLevel().getBlock(x + face.getXOffset() * step,
                            y + face.getYOffset() * step, z + face.getZOffset() * step, layer);
                } else {
